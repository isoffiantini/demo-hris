...
const { resolveTrackingCode, resolveRecordId, sendErrorLog, utcDateTime, resolveLogContext, makeLog, sendLogSafe } = require("./executionHistory");
const { attachForm, moveApplicationToStep } = require("./hrisSync");
...
function toEmployeePayload(payload) {
  ...
  return {
    ...
    employmentStatus: EMPLOYMENT_STATUS_VALUES.includes(props.employmentStatus) ? props.employmentStatus : EMPLOYMENT_STATUS_VALUES[0],
...
function upsertEmployee(employee, avaturePersonId) {
  ...
  const storedPersonId = avaturePersonId === null || avaturePersonId === undefined || avaturePersonId === "" ? null : avaturePersonId;

  if (existing) {
    const updated = { ...existing, ...employee, department, id: existing.id, avaturePersonId: existing.avaturePersonId || storedPersonId };
...
function resolveApplicationId(payload) {
  ...
  for (const candidate of [props.application_id, props.applicationId, payload.application_id]) {
...
function validateFlowInput(employee) {
  ...
  for (const field of ["firstName", "lastName", "email"]) {
    if (!employee[field] || String(employee[field]).trim() === "") errors.push(`${field} is required`);
  }
  return errors;
}
...
function persistSyncFormId(recordId, formId) {
  ...
  if (employee && String(employee.avatureSyncFormId || "") !== String(formId)) {
    data.employees = data.employees.map((e) => (e.id === recordId ? { ...e, avatureSyncFormId: formId } : e));
    writeData(data);
...
router.get("/flow_create_employee", (req, res) => {
  res.json({ "avature-challenge-code": req.get("avature-challenge-code") || "" });
});
...
router.post("/flow_create_employee", async (req, res) => {
  try {
    ...
    if (validationErrors.length) {
      ...
      void sendErrorLog(req, { recordTypeId: RECORD_TYPE_EMPLOYEE, message: `Validation failed: ${validationErrors.join(", ")}` });
      ...
      return res.status(400).json({ asyncResponse: { successful: false, errors: validationErrors } });
    }
    ...
    const infoSummary = created ? "Employee successfully created in the HRIS." : "Employee email already exists - record updated.";
    ...
    console.log(`[flow_create_employee] trackingCode="${resolveTrackingCode(req)}" recordIdFromPayload=${JSON.stringify(requestPersonId)} avaturePersonId=${JSON.stringify(personId)} applicationId="${applicationId}" body=${JSON.stringify(req.body || {})}`);
    ...
    void (async () => {
      ...
      if (personId === null || personId === "") {
        console.warn(`[hrisSync] No Avature person id (recordId) in request; skipping form sync. Full body: ${JSON.stringify(req.body || {})}`);
        await sendLogSafe(makeLog(ctx, RECORD_TYPE_EMPLOYEE, "Flow finished successfully.", "Execution was successful.", "SUCCESS"));
        return;
      }
      try {
        const result = await attachForm(personId, { hrisExternalId: record.id, hrisUrl: employeeUrl(req, record.id), syncDetails: "Success", lastSynced: utcDateTime() });
        persistSyncFormId(record.id, result.formId);
        if (result.action === "skipped") {
          console.warn(`[hrisSync] Form sync skipped: ${result.reason}`);
          await sendLogSafe(makeLog(ctx, RECORD_TYPE_EMPLOYEE, "Flow failed.", `Execution failed: form sync skipped (${result.reason}).`, "ERROR"));
          return;
        }
        const actionText = result.action === "created" ? "created" : "updated";
        await sendLogSafe(makeLog(ctx, RECORD_TYPE_EMPLOYEE, "Person record synced into Avature.", `The person record has been synced into Avature: the sync form attached to the person was ${actionText} successfully.`, "INFO"));
        if (applicationId) {
          try {
            await moveApplicationToStep(applicationId, APPLICATION_STEP_HIRED);
            await sendLogSafe(makeLog(ctx, RECORD_TYPE_EMPLOYEE, "Candidate moved to next workflow step.", `The candidate was moved to workflow step ${APPLICATION_STEP_HIRED} (application id ${applicationId}).`, "INFO"));
          } catch (moveErr) {
            console.error(`[flow_create_employee] Failed to move application ${applicationId} to step ${APPLICATION_STEP_HIRED}: ${moveErr.message}`);
            await sendLogSafe(makeLog(ctx, RECORD_TYPE_EMPLOYEE, "Flow failed.", `Execution failed: could not move application ${applicationId} to workflow step ${APPLICATION_STEP_HIRED} (${moveErr.message}).`, "ERROR"));
            return;
          }
        } else {
        ...
        await sendLogSafe(makeLog(ctx, RECORD_TYPE_EMPLOYEE, "Flow failed.", `Execution failed: ${err.message}`, "ERROR"));
      }
    })();
  } catch (err) {
